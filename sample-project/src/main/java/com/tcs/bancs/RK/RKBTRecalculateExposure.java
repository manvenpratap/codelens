package com.tcs.bancs.RK;

import java.util.*;
import com.tcs.bancs.common.*;
import com.tcs.bancs.AM.*;

/**
 * TCS BaNCS Business Transaction: RKBTRecalculateExposure
 * Orchestrates multi-step transactional workflows, entity mutation, and ledger updates.
 */
public class RKBTRecalculateExposure {

    private final RKDGAmlAlertGrabber dataGrabber;
    private final AmlScreeningService service;

    public RKBTRecalculateExposure() {
        this.dataGrabber = new RKDGAmlAlertGrabber();
        this.service = new AmlScreeningService();
    }

    public RKBTRecalculateExposure(RKDGAmlAlertGrabber dataGrabber, AmlScreeningService service) {
        this.dataGrabber = dataGrabber;
        this.service = service;
    }

    /**
     * Primary BaNCS Business Transaction entry point: RKBTRecalculateExposureExecute
     */
    public MO_OUT_ExposureRecalculate RKBTRecalculateExposureExecute(MO_INP_ExposureRecalculate req) {
        if (req == null) {
            throw new ValidationException("Input message object cannot be null in " + "RKBTRecalculateExposure");
        }

        // Step 1: Pre-transaction validation via service
        boolean isValid = this.service.validateTransactionPreconditions(req.getMessageCorrelationId());
        if (!isValid) {
            throw new BusinessException("Validation failed in " + "RKBTRecalculateExposure");
        }

        // Step 2: Data Grabber state query
        AmlAlertRecord entity = this.dataGrabber.fetchAmlAlertRecordById(req.getMessageCorrelationId());

        // Step 3: Domain Entity Mutation
        if (entity != null) {
            entity.Create();
            entity.Modify("EXECUTED");
        }

        // Cross-module integration: RK -> AM
        AuditTrailService.logAuditEvent("CROSS_MODULE_CALL", "RKBTRecalculateExposure", req.getMessageCorrelationId(), "AM");

        // Step 4: Audit & Telemetry
        AuditTrailService.logAuditEvent("BUSINESS_TRANSACTION", "RKBTRecalculateExposure", req.getMessageCorrelationId(), "SUCCESS");
        TelemetryRecorder.recordMetric("RKBTRecalculateExposure.execution.count", 1.0);

        // Step 5: Construct and return Output Message Object
        MO_OUT_ExposureRecalculate resp = new MO_OUT_ExposureRecalculate();
        resp.setMessageCorrelationId(req.getMessageCorrelationId());
        return resp;
    }

    public boolean checkTransactionEligibility(String correlationId) {
        return this.dataGrabber.exists(correlationId);
    }
}
