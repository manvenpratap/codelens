package com.tcs.bancs.RK;

import java.util.*;
import com.tcs.bancs.common.*;
import com.tcs.bancs.AM.*;

/**
 * TCS BaNCS Business Transaction: RKBTApproveRiskOverride
 * Orchestrates multi-step transactional workflows, entity mutation, and ledger updates.
 */
public class RKBTApproveRiskOverride {

    private final RKDGExposureGrabber dataGrabber;
    private final LimitManagementService service;

    public RKBTApproveRiskOverride() {
        this.dataGrabber = new RKDGExposureGrabber();
        this.service = new LimitManagementService();
    }

    public RKBTApproveRiskOverride(RKDGExposureGrabber dataGrabber, LimitManagementService service) {
        this.dataGrabber = dataGrabber;
        this.service = service;
    }

    /**
     * Primary BaNCS Business Transaction entry point: RKBTApproveRiskOverrideExecute
     */
    public MO_OUT_RiskOverride RKBTApproveRiskOverrideExecute(MO_INP_RiskOverride req) {
        if (req == null) {
            throw new ValidationException("Input message object cannot be null in " + "RKBTApproveRiskOverride");
        }

        // Step 1: Pre-transaction validation via service
        boolean isValid = this.service.validateTransactionPreconditions(req.getMessageCorrelationId());
        if (!isValid) {
            throw new BusinessException("Validation failed in " + "RKBTApproveRiskOverride");
        }

        // Step 2: Data Grabber state query
        VaRCalculationResult entity = this.dataGrabber.fetchVaRCalculationResultById(req.getMessageCorrelationId());

        // Step 3: Domain Entity Mutation
        if (entity != null) {
            entity.Create();
            entity.Modify("EXECUTED");
        }

        // Cross-module integration: RK -> AM
        AuditTrailService.logAuditEvent("CROSS_MODULE_CALL", "RKBTApproveRiskOverride", req.getMessageCorrelationId(), "AM");

        // Step 4: Audit & Telemetry
        AuditTrailService.logAuditEvent("BUSINESS_TRANSACTION", "RKBTApproveRiskOverride", req.getMessageCorrelationId(), "SUCCESS");
        TelemetryRecorder.recordMetric("RKBTApproveRiskOverride.execution.count", 1.0);

        // Step 5: Construct and return Output Message Object
        MO_OUT_RiskOverride resp = new MO_OUT_RiskOverride();
        resp.setMessageCorrelationId(req.getMessageCorrelationId());
        return resp;
    }

    public boolean checkTransactionEligibility(String correlationId) {
        return this.dataGrabber.exists(correlationId);
    }
}
