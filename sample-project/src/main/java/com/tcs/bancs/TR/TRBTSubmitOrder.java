package com.tcs.bancs.TR;

import java.util.*;
import com.tcs.bancs.common.*;
import com.tcs.bancs.CL.*;

/**
 * TCS BaNCS Business Transaction: TRBTSubmitOrder
 * Orchestrates multi-step transactional workflows, entity mutation, and ledger updates.
 */
public class TRBTSubmitOrder {

    private final TRDGTradeGrabber dataGrabber;
    private final OrderRoutingService service;

    public TRBTSubmitOrder() {
        this.dataGrabber = new TRDGTradeGrabber();
        this.service = new OrderRoutingService();
    }

    public TRBTSubmitOrder(TRDGTradeGrabber dataGrabber, OrderRoutingService service) {
        this.dataGrabber = dataGrabber;
        this.service = service;
    }

    /**
     * Primary BaNCS Business Transaction entry point: TRBTSubmitOrderExecute
     */
    public MO_OUT_OrderSubmission TRBTSubmitOrderExecute(MO_INP_OrderSubmission req) {
        if (req == null) {
            throw new ValidationException("Input message object cannot be null in " + "TRBTSubmitOrder");
        }

        // Step 1: Pre-transaction validation via service
        boolean isValid = this.service.validateTransactionPreconditions(req.getMessageCorrelationId());
        if (!isValid) {
            throw new BusinessException("Validation failed in " + "TRBTSubmitOrder");
        }

        // Step 2: Data Grabber state query
        OrderEntity entity = this.dataGrabber.fetchOrderEntityById(req.getMessageCorrelationId());

        // Step 3: Domain Entity Mutation
        if (entity != null) {
            entity.Create();
            entity.Modify("EXECUTED");
        }

        // Cross-module integration: TR -> CL
        AuditTrailService.logAuditEvent("CROSS_MODULE_CALL", "TRBTSubmitOrder", req.getMessageCorrelationId(), "CL");

        // Step 4: Audit & Telemetry
        AuditTrailService.logAuditEvent("BUSINESS_TRANSACTION", "TRBTSubmitOrder", req.getMessageCorrelationId(), "SUCCESS");
        TelemetryRecorder.recordMetric("TRBTSubmitOrder.execution.count", 1.0);

        // Step 5: Construct and return Output Message Object
        MO_OUT_OrderSubmission resp = new MO_OUT_OrderSubmission();
        resp.setMessageCorrelationId(req.getMessageCorrelationId());
        return resp;
    }

    public boolean checkTransactionEligibility(String correlationId) {
        return this.dataGrabber.exists(correlationId);
    }
}
