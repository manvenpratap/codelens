package com.tcs.bancs.AM;

import java.util.*;
import com.tcs.bancs.common.*;
import com.tcs.bancs.GL.*;

/**
 * TCS BaNCS Business Transaction: AMBTApplyCharge
 * Orchestrates multi-step transactional workflows, entity mutation, and ledger updates.
 */
public class AMBTApplyCharge {

    private final AMDGStatementGrabber dataGrabber;
    private final AccountLifecycleService service;

    public AMBTApplyCharge() {
        this.dataGrabber = new AMDGStatementGrabber();
        this.service = new AccountLifecycleService();
    }

    public AMBTApplyCharge(AMDGStatementGrabber dataGrabber, AccountLifecycleService service) {
        this.dataGrabber = dataGrabber;
        this.service = service;
    }

    /**
     * Primary BaNCS Business Transaction entry point: AMBTApplyChargeExecute
     */
    public MO_OUT_AccountClosure AMBTApplyChargeExecute(MO_INP_AccountClosure req) {
        if (req == null) {
            throw new ValidationException("Input message object cannot be null in " + "AMBTApplyCharge");
        }

        // Step 1: Pre-transaction validation via service
        boolean isValid = this.service.validateTransactionPreconditions(req.getMessageCorrelationId());
        if (!isValid) {
            throw new BusinessException("Validation failed in " + "AMBTApplyCharge");
        }

        // Step 2: Data Grabber state query
        AccountFeeSchedule entity = this.dataGrabber.fetchAccountFeeScheduleById(req.getMessageCorrelationId());

        // Step 3: Domain Entity Mutation
        if (entity != null) {
            entity.Create();
            entity.Modify("EXECUTED");
        }

        // Cross-module integration: AM -> GL
        AuditTrailService.logAuditEvent("CROSS_MODULE_CALL", "AMBTApplyCharge", req.getMessageCorrelationId(), "GL");

        // Step 4: Audit & Telemetry
        AuditTrailService.logAuditEvent("BUSINESS_TRANSACTION", "AMBTApplyCharge", req.getMessageCorrelationId(), "SUCCESS");
        TelemetryRecorder.recordMetric("AMBTApplyCharge.execution.count", 1.0);

        // Step 5: Construct and return Output Message Object
        MO_OUT_AccountClosure resp = new MO_OUT_AccountClosure();
        resp.setMessageCorrelationId(req.getMessageCorrelationId());
        return resp;
    }

    public boolean checkTransactionEligibility(String correlationId) {
        return this.dataGrabber.exists(correlationId);
    }
}
